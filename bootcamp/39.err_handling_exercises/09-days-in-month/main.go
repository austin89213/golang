...
func main() {
	...
	monthDaysMap := map[string]int{
		"january": 31, "feburary": 28, "march": 31, "april": 30,
		"may": 31, "june": 30, "july": 31, "august": 31,
		"september": 30, "october": 31, "november": 30, "december": 31}
	year := time.Now().Year()
	if year%4 == 0 && (year%100 != 0 || year%400 == 0) {
		monthDaysMap["feburary"] = 29
	}

	month := strings.ToLower(os.Args[1])
	if days, ok := monthDaysMap[month]; ok {
		//(days, ok) would be like (0, false) or (31, ture), depends on if the input value exists in the dict
